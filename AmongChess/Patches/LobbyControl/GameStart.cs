...
namespace AmongChess.Patches.LobbyControl
{
	internal class GameStart
	{
		...
		public static class GameStartManagerPatch
		{
			...
			{
				OptionSingle GameMode = OptionControl.AllOption.Find(ele => ele.Name == "Game Mode");
		...
		private class PlayerControlPatch
		{
			...
			public static void RpcSetInfected()
			{
				...
				{
					GameControl.Control.AllPlayers = new List<PlayerControl> { };
					int playersCount = GameData.Instance.PlayerCount;
					int[] colorsArray = (int[])GameControl.Control.ColorIds.GetValue(playersCount - 1);
