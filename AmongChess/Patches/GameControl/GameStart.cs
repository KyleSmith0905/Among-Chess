	...
	internal class GameStart
	{
		...
		public static class IntroCutScenePatch
		{
			...
			public static void BeginCrewmatePatch(IntroCutscene __instance)
			{
				...
				if (Control.AllPlayers.Count == 0)
				{
					float timeAdded = ChessControl.Control.MainTime != "Unlimited" ? float.Parse(ChessControl.Control.MainTime) * 60 : float.MaxValue;
					for (int i = 0; i < colorIds.Length; i++)
					{
						for (int j = 0; j < PlayerControl.AllPlayerControls.Count; j++)
						{
							...
							Control.AllPlayers.Add(playerControl);
							...
							Control.AllCustomPlayers.Add(customPlayer);
						}
					}
				}
				...
				int index = -1;
				string otherTeams = "";
				for (int i = 0; i < Control.AllPlayers.Count; i++) if (Control.AllPlayers[i].PlayerId == PlayerControl.LocalPlayer.PlayerId) index = i;
				for (int i = 0; i < playerCount - 1; i++)
				{
					if (i == index) continue;
					otherTeams = otherTeams + ", " + Control.ColorNames[colorIds[i]].ToString().ToLower();
				}
				...
				__instance.Title.text = Control.ColorNames[colorIds[index]].ToString();
		...
		public static class ShipStatusPatch
		{
			public static void Postfix()
			{
				...
				ChessControl.Control.ChessBoard = new char[,] { { '0' } };
				ChessControl.Control.SetSettings();
				char[,] chessBoard = ChessControl.Utilities.ReadableBoard(ChessControl.Control.ChessBoard);
				int[] allColors = (int[])Control.ColorIds.GetValue(GameData.Instance.PlayerCount - 1);
				for (int y = 0; y < 8; y++)
				{
					for (int x = 0; x < 8; x++)
					{
						...
						int pieceIndex = Utilities.PieceIndex(chessBoard[y, x]);
						...
						playerControl.SetHat(Utilities.PieceHats[pieceIndex], allColors[team]);
						playerControl.SetSkin(Utilities.PieceSkins[pieceIndex]);
