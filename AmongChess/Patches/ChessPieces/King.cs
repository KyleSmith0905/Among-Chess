	...
	internal class King
	{
		public static bool CanMove((int x, int y) fromCoordinates, (int x, int y) toCoordinates, ref ChessControl.EnumMoves howMove)
		{
			...
			for (int index = 0; index < 8; index++)
			{
				...
				if (ChessControl.Validation.CheckBounds(testCoordinates)) continue;
			...
			if (char.ToUpper(chessBoard[fromCoordinates.y, fromCoordinates.x]) == 'M' && char.ToUpper(chessBoard[toCoordinates.y, toCoordinates.x]) == 'H' && fromCoordinates.y == toCoordinates.y)
			{
				...
				for (int i = 0; i < 8; i++)
				{
					...
					if (ChessControl.Validation.CheckBounds(testCoordinates)) return false;
					...
					if (ChessControl.Validation.NumCheck(testCoordinates, tempChessBoard).Count > 0)
					{
						return false;
					}
					else if (testCoordinates.x == endPosition)
					{
						howMove = directionX == 1 ? ChessControl.EnumMoves.KingCastle : ChessControl.EnumMoves.QueenCastle;
						return true;
					}
