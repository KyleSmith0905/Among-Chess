...
namespace AmongChess.Source.Chess
{
	internal class Utils
	{
		...
		public static EnumMoves GetHowMove((int x, int y) fromCoordinates, (int x, int y) toCoordinates)
		{
			...
			int directionY = char.IsUpper(Chess.ChessBoard[fromCoordinates.y, fromCoordinates.x]) ? -1 : 1;
			float halfRank = (Chess.ChessBoard.GetLength(0) * 0.5f) - 0.5f;
			if (char.ToUpper(Chess.ChessBoard[fromCoordinates.y, fromCoordinates.x]) == 'M' && char.ToUpper(Chess.ChessBoard[toCoordinates.y, toCoordinates.x]) == 'H' && fromCoordinates.y == toCoordinates.y) howMove = Math.Sign(toCoordinates.x - fromCoordinates.x) == 1 ? EnumMoves.KingCastle : EnumMoves.QueenCastle;
			else if (char.ToUpper(ReadablePiece(Chess.ChessBoard[fromCoordinates.y, fromCoordinates.x])) == 'P' && char.ToUpper(Chess.ChessBoard[fromCoordinates.y, toCoordinates.x]) == 'E') howMove = EnumMoves.EnPassant;
			else if (char.ToUpper(ReadablePiece(Chess.ChessBoard[fromCoordinates.y, fromCoordinates.x])) == 'P' && toCoordinates.y == halfRank + (halfRank * directionY)) howMove = EnumMoves.Promotion;
			return howMove;
		}
	}
}