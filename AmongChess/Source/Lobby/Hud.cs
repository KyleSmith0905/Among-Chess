	...
	internal class Hud
	{
		...
		public static class GameOptionsDataPatch
		{
			...
			public static bool ToHudString(ref string __result, ref GameOptionsData __instance)
			{
				...
				{
					ClassOption optionSingle = Options.AllOption.Find(ele => ele.Id == Ids[i]);
					if (!optionSingle.Exemption.Equals(default))
					{
						ClassOption exemptionOption = Options.AllOption.Find(ele => ele.Id == optionSingle.Exemption.Id);
					...
					else
					{
						...
						ClassOptionGroup optionGroupSingle = Options.AllOptionGroup.Find(ele => ele.Id == optionSingle.GroupId);
						...
						if (!optionGroupSingle.Exemption.Equals(default))
						{
							...
							ClassOption exemptionOption = Options.AllOption.Find(ele => ele.Id == optionGroupSingle.Exemption.OptionId);
						...
						for (int j = 0; j < numVariables; j++)
						{
							...
							ClassOption variableOption = Options.AllOption.Find(ele => ele.Id == optionId);
